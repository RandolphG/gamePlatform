...
export class Engine implements IEngine {
  ...
  init = () => {
    let shadow = this.ctx.createLinearGradient(
      0,
      this.WALL_HEIGHT,
      0,
      this.HEIGHT
    );
    let now = this.ctx.createLinearGradient(0, 0, 0, this.WALL_HEIGHT);
  ...
  resizeWalls = () => {
    ...
    let countRep = this.walls.length;
