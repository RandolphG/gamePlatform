...
export class Hedgehog implements IHedgehog {
  ...
  init(): void {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      const sb = new THREE.Mesh(spikeGeom, this.material.blackMat)
      sb.rotation.x = -Math.PI / 2 + (Math.PI / 12) * row - 0.5 + Math.random()
      sb.position.z = -3
      sb.position.y = -2 + row * 2
      sb.position.x = -2 + col * 2
      this.body.add(sb)
      const st = new THREE.Mesh(spikeGeom, this.material.blackMat)
      st.position.y = 3
      ...
      const sr = new THREE.Mesh(spikeGeom, this.material.blackMat)
      sr.position.x = 3
      ...
      const sl = new THREE.Mesh(spikeGeom, this.material.blackMat)
      sl.position.x = -3
