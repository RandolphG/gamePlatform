...
export class Carrot implements ICarrot {
  ...
  init(): void {
    ...
    /* testGeometry */
    const cylinderGeom = new THREE.CylinderGeometry(5, 3, 10, 4, 1)
    const cylinderBodyBufferGeom = new THREE.CylinderBufferGeometry()
    ...
    const boxGeom = new THREE.BoxGeometry(5, 3, 10, 4, 1)
    const boxLeafBufferGeom = new THREE.BoxBufferGeometry()

    boxLeafBufferGeom.merge(boxGeom)
    boxLeafBufferGeom.applyMatrix4(new THREE.Matrix4().makeTranslation(0, 5, 0))
